...
export default function BottomSheet({ open, title, subtitle, children, footer, onClose }) {
  useEffect(() => {
    if (!open) return
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000000004]">
      <button type="button" className="absolute inset-0 bg-black/55 backdrop-blur-[1px]" aria-label="Cerrar" onClick={onClose} />

      <div
        role="dialog"
        aria-modal="true"
        className="absolute inset-x-0 bottom-0 max-h-[85vh] w-full rounded-t-3xl bg-surface/95 text-text shadow-2xl ring-1 ring-border/20 backdrop-blur"
      >
        <div className="flex max-h-[85vh] flex-col">
          <div className="flex items-start justify-between gap-3 border-b border-border/15 px-5 py-4">
          ...
          {footer ? (
            <div className="border-t border-border/15 bg-surface/60 px-5 py-4 pb-[calc(env(safe-area-inset-bottom)+16px)]">{footer}</div>
          ) : null}
        </div>
      </div>
    </div>,
