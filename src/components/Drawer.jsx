...
export default function Drawer({ open, title, subtitle, children, footer, onClose }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000000003]">
      <button type="button" className="absolute inset-0 bg-black/55 backdrop-blur-[1px]" aria-label="Cerrar" onClick={onClose} />

      <div
        role="dialog"
        aria-modal="true"
        className="absolute inset-y-0 right-0 w-full max-w-[640px] bg-surface/95 text-text shadow-2xl ring-1 ring-border/20 backdrop-blur"
      >
        <div className="flex h-full flex-col">
          <div className="flex items-start justify-between gap-3 border-b border-border/15 px-6 py-5">
          ...
          {footer ? <div className="border-t border-border/15 bg-surface/60 px-6 py-4">{footer}</div> : null}
        </div>
      </div>
    </div>,
    document.body
