...
import { guardarProducto, eliminarProducto } from '@/firebase/ops'
...
export default function ProductsPage() {
      ...
      <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
        <label className="inline-flex items-center gap-2 text-[12px] text-muted">
          ...
          >
            ...
            <option value="codigoLower">CÃ³digo</option>
          </select>
        </label>
  ...
  return (
    ...
    >
      ...
      {modal === 'Delete' && (
        <Modal funcion={deleteConfirm}>
          ¿Eliminar el producto <span className="font-semibold">{msg || item?.nombre || item?.__key}</span>?
        </Modal>
      )}
